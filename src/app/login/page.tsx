...
import { loginWithPhone, sendSMSCode, getCurrentUser, getUserLevel, getNextLevel, XP_RULES, logout, User } from "@/data/user"
...
export default function LoginPage() {
      ...
      <div style={{background:'#000',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Noto Sans SC', sans-serif"}}>
        <div style={{background:'rgba(255,255,255,0.02)',border:'1px solid #1a1a1a',padding:48,maxWidth:380,width:'100%',textAlign:'center'}}>
          ...
          <div style={{textAlign:'left',marginTop:20}}>
            <p style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:'#555',marginBottom:12}}>XP RULES</p>
  ...
  const sendCode = async () => {
    ...
    setError("")
    const c = await sendSMSCode(phone)
    setMockCode(c); setCountdown(60); setStep("code")
  }
    ...
    <div style={{background:'#000',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Noto Sans SC', sans-serif"}}>
      <div style={{background:'rgba(255,255,255,0.02)',border:'1px solid #1a1a1a',padding:48,maxWidth:380,width:'100%'}}>
        ...
        <div style={{display:'flex',justifyContent:'center',gap:0,marginBottom:32}}>
          {["phone","code","nickname"].map((s,i)=>(
            <div key={s} style={{display:'flex',alignItems:'center',gap:0}}>
              <div style={{width:28,height:28,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'JetBrains Mono',monospace",fontSize:11,fontWeight:700,background:step===s?'rgba(201,168,76,0.15)':i<["phone","code","nickname"].indexOf(step)?'rgba(201,168,76,0.05)':'transparent',color:step===s?'#c9a84c':i<["phone","code","nickname"].indexOf(step)?'#7a6230':'#333',border:`1px solid ${step===s?'#7a6230':'#1a1a1a'}`}}>{i+1}</div>
        ...
        {step==="phone"&&<div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          <button onClick={sendCode} className="btn-primary" style={{justifyContent:'center'}}>获取验证码</button>
        </div>}
        ...
        {step==="code"&&<div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          {error&&<p style={{fontSize:11,color:'#D94841'}}>{error}</p>}
          <button onClick={()=>{if(code!==mockCode){setError("验证码错误");return}setError("");setStep("nickname")}} className="btn-primary" style={{justifyContent:'center'}}>验证</button>
        ...
        {step==="nickname"&&<div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          {error&&<p style={{fontSize:11,color:'#D94841'}}>{error}</p>}
          <button onClick={async()=>{setError("");const u=await loginWithPhone(phone,name);if(!u){setError("登录失败，请重试");return};refresh()}} className="btn-primary" style={{justifyContent:'center'}}>开始探索</button>
        </div>}
        <p style={{fontSize:9,color:'#333',textAlign:'center',marginTop:16,fontFamily:"'JetBrains Mono',monospace"}}>演示模式 · 真实上线后对接短信服务</p>
      </div>
    </div>
  )
}
