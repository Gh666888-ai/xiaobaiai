...
6. 优先引导用户使用站内页面，但不要把 /choose-tool、/learn、/models 这种技术路径直接说给新手；请说中文页面名，例如「AI 工具选择器」「小白爱学习」「模型排行」「社区案例」「成长舱」，并告诉用户点哪里。
...
function siteReply(message: string) {
  ...
  if (text.includes("你是谁") || text.includes("叫什么") || text.includes("deepseek") || text.includes("模型")) {
    return `我是小白AI网站里的小白AI助手，负责帮你选 AI 工具、找学习路径、看模型排行、理解资讯和拆解需求。简单说：你提目标，我把路点亮。\n\n${siteIntro}\n\n站内最常用的入口：\n1. 「AI 工具选择器」：回答几个问题，帮你选工具。\n2. 「小白爱学习」：从零开始学 AI 和 Agent。\n3. 「模型排行」：看更新时间、价格口径和适合场景。\n4. 「社区案例」：看真实案例、踩坑记录和投稿。\n5. 「成长舱」：每天打卡，积累学习进度。\n\n你可以直接说“我想做什么”，我会先给你最短路线。`
  }
