export type AppTemplateId = "site-hero" | "lead-form" | "quote-calculator" | "product-page" | "click-game" | "chat-app"
...
type AppMode = {
  ...
  heroDesc: string
  systemName: string
  visual: "radar" | "pipeline" | "calculator" | "commerce" | "arcade" | "chat"
}
...
export const appTemplates: AppTemplate[] = [
  ...
  },
  {
    ...
    title: "聊天社交应用",
    shortTitle: "聊天应用",
    description: "做类似微信的聊天软件：消息列表、聊天窗口、通讯录、发现、我的和发送消息。",
    bestFor: "社交、客服、私信、社群、内部沟通",
    defaultGoal: "做一个像微信一样能看会话、选联系人、发送消息的聊天软件",
    accent: "#35d07f",
  },
]
...
const appModes: Record<AppTemplateId, AppMode> = {
  ...
  "chat-app": {
    ...
    name: "聊天社交应用",
    activePage: "chat",
    heroTitle: "做一个能直接聊天的消息应用",
    heroDesc: "这不是获客页，而是一个基础聊天软件原型：左侧会话列表，右侧聊天窗口，底部能输入并发送消息。",
    systemName: "即时通讯",
    visual: "chat",
  },
}
...
function css(colors: Palette, mode: AppMode) {
  const pageBg =
    mode.visual === "chat"
      ? `radial-gradient(circle at 20% 8%,${colors.accent}26,transparent 28%),linear-gradient(135deg,rgba(7,28,20,.75),rgba(8,12,16,.9))`
...
function visualPanel(mode: AppMode, blueprint: BusinessBlueprint, industry: string, contact: string) {
  if (mode.visual === "chat") {
    return `<aside class="panel visual float"><div class="device"><p class="eyebrow">${safe(mode.systemName)}</p><h2 style="font-size:32px;line-height:1.08;margin:0">消息中枢</h2><div class="card"><b>会话列表</b><span class="muted">好友、群聊、客服消息按最新时间排序。</span></div><div class="card"><b>聊天窗口</b><span class="muted">支持查看消息、输入文字、点击发送。</span></div><div class="card"><b>通讯录/发现/我的</b><span class="muted">基础微信式导航都已经放进原型。</span></div><a class="btn" href="#chat-demo">${contact}</a></div></aside>`
  }
...
function chatAppHtml(industry: string, contact: string, colors: Palette, mode: AppMode, sheet: string) {
  const contacts = [
    { id: "xiaobai", name: "小白AI", desc: "我已经帮你搭好聊天应用原型", time: "09:41", avatar: "白" },
    { id: "team", name: "项目讨论群", desc: "今晚把登录和消息发送跑通", time: "09:36", avatar: "群" },
    { id: "service", name: "客户咨询", desc: "用户问能不能做成小程序", time: "昨天", avatar: "客" },
    { id: "friend", name: "好友阿众", desc: "这个界面已经像聊天软件了", time: "周五", avatar: "众" },
  ]
  const contactList = contacts
    .map(
      (item, index) =>
        `<button class="chat-item${index === 0 ? " active" : ""}" type="button" data-chat="${safe(item.id)}"><span class="avatar">${safe(item.avatar)}</span><span><b>${safe(item.name)}</b><span class="muted" style="display:block;font-size:13px">${safe(item.desc)}</span></span><span class="muted" style="font-size:12px">${safe(item.time)}</span></button>`,
    )
    .join("")
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${industry}聊天应用</title><style>${sheet}</style></head><body><main class="page"><span class="orb a"></span><span class="orb b"></span><div class="shell">${nav(industry, contact)}<section class="hero"><div><p class="eyebrow">${safe(mode.name)} · 微信式消息原型</p><h1 class="title">${safe(mode.heroTitle)}</h1><p class="desc">${safe(mode.heroDesc)} 当前版本已经包含会话列表、聊天窗口、联系人、发现、我的和发送消息。</p><div class="metric-row"><div class="metric"><b>4个</b><span>默认会话</span></div><div class="metric"><b>5页</b><span>消息/通讯录/发现/我的/设置</span></div><div class="metric"><b>可发送</b><span>输入文字实时追加</span></div></div><div class="actions"><a class="btn" href="#chat-demo">打开聊天应用</a><a class="btn ghost" href="#features">看功能结构</a></div></div>${visualPanel(mode, { sector: "聊天社交", coreOffer: "即时通讯", hook: "", userPain: "", trustProof: [], serviceSteps: [], captureFields: [], packages: [], appGoal: "", nextWorkflow: "" }, industry, contact)}</section><section class="section" id="chat-demo"><h2 class="section-title">微信式聊天应用原型</h2><p class="desc">这才是聊天软件应该先出现的东西：左边会话列表，右边聊天窗口，底部能输入并发送消息。</p><div class="chat-app"><aside class="chat-sidebar"><div class="chat-search">搜索联系人 / 群聊 / 聊天记录</div>${contactList}</aside><section class="chat-main"><header class="chat-head"><div><b id="chat-title">小白AI</b><span class="muted" style="display:block;font-size:13px">在线 · 正在生成应用</span></div><button class="btn ghost" type="button">···</button></header><div class="messages" id="messages"><div class="bubble left">你想做一个微信一样的聊天软件，我会先搭出消息列表和聊天窗口。</div><div class="bubble right">要能像微信那样看会话、点联系人、发消息。</div><div class="bubble left">可以。这个原型已经支持输入并发送消息，后面再接账号、数据库和实时通信。</div></div><div class="composer"><button type="button">＋</button><input id="message-input" placeholder="输入消息，按发送追加到聊天窗口"/><button type="button" onclick="sendMessage()">发送</button></div></section></div><div class="chat-mobile" style="margin-top:18px"><div class="mobile-top">移动端底部导航</div><div class="messages"><div class="bubble left">消息页面</div><div class="bubble right">通讯录、发现、我的也要有。</div></div><div class="mobile-tabs"><button class="active">消息</button><button>通讯录</button><button>发现</button><button>我的</button></div></div></section><section class="section two" id="features"><div><h2 class="section-title">基础功能结构</h2><div class="steps"><div class="step"><div><b>会话列表</b><span class="muted">显示好友、群聊、客服消息和最后一条消息。</span></div></div><div class="step"><div><b>聊天窗口</b><span class="muted">消息左右气泡、顶部资料、底部输入栏。</span></div></div><div class="step"><div><b>通讯录</b><span class="muted">联系人、群聊、新朋友、标签入口。</span></div></div><div class="step"><div><b>发现 / 我的</b><span class="muted">朋友圈、扫一扫、收藏、设置等入口。</span></div></div></div></div><div class="panel"><p class="eyebrow">下一步升级</p><div class="card"><b>真正可用还需要什么</b><span class="muted">账号登录、消息数据库、WebSocket 实时通信、已读未读、图片语音、好友关系和后台管理。</span></div><div class="card" style="margin-top:12px"><b>当前这版先解决什么</b><span class="muted">先让用户看到“这确实是聊天软件”，不是拿获客页糊弄。</span></div></div></section></div></main><script>document.querySelectorAll('.chat-item').forEach(function(item){item.addEventListener('click',function(){document.querySelectorAll('.chat-item').forEach(function(x){x.classList.remove('active')});item.classList.add('active');var title=item.querySelector('b');document.getElementById('chat-title').textContent=title?title.textContent:'聊天';var box=document.getElementById('messages');box.innerHTML='<div class="bubble left">这里是和 '+document.getElementById('chat-title').textContent+' 的聊天。</div><div class="bubble right">这个会话可以切换，也可以继续发送消息。</div>'})});function sendMessage(){var input=document.getElementById('message-input');var text=(input.value||'').trim();if(!text)return;var box=document.getElementById('messages');var bubble=document.createElement('div');bubble.className='bubble right';bubble.textContent=text;box.appendChild(bubble);input.value='';box.scrollTop=box.scrollHeight}</script></body></html>`
}
...
export function generateAppHtml(input: GeneratedAppInput) {
  ...
  if (template.id === "chat-app") {
    return chatAppHtml(industry, contact, colors, mode, sheet)
  }
