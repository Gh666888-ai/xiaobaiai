...
export const stages: Stage[] = [
  ...
  {id:2,title:"安装主流Agent",subtitle:"在电脑上安装Agent，让它帮你自动干活",
    ...
    {title:"Agent 桌面端 App 怎么安装和设置？",
      content:"桌面端 App 和 Agent 先分清产品方官方、第三方、插件、本地模型，不要混在一起装。\n\n第一类：产品方官方 Agent 桌面端\n• Codex App：OpenAI Codex 官方桌面端，用来在本机管理工程 Agent、读项目、看 diff、跑验证和恢复长任务。\n• Cursor、Windsurf、Kiro、TRAE、Qoder 也属于产品方官方 AI IDE / Agent 桌面应用。\n• 教程入口：/agent-install/codex-app、/agent-install/cursor-agent、/agent-install/windsurf。\n\n第二类：工程 Agent / CLI\n• Claude Code、Codex CLI、Gemini CLI、Qwen Code、Aider：更适合会一点终端和 Git 的用户。\n• 它们能读项目、改代码、跑命令，但需要更重视权限和验证。\n• 教程入口：/agent-install/claude-code、/agent-install/codex、/agent-install/gemini-cli、/agent-install/qwen-code、/agent-install/aider。\n\n第三类：IDE Agent 插件\n• GitHub Copilot Agent、Cline、Continue、Augment、JetBrains Junie：装在 VS Code 或 JetBrains 里。\n• 插件不是模型，模型/API/账号要在插件或产品设置里配置。\n• 教程入口：/agent-install/github-copilot-agent、/agent-install/cline、/agent-install/continue-dev、/agent-install/augment-code。\n\n第四类：第三方 Agent 桌面端\n• ClawX / Claw Desktop：连接 OpenClaw 本体，用来看会话、审批操作、看 diff、恢复任务。\n• 这里填的是 OpenClaw Host URL 和 OpenClaw API Key，不是 DeepSeek Key。\n• 先去 /agent-install/openclaw 跑通 OpenClaw，再去 /agent-install/clawx 连接桌面端。\n\n第五类：产品方官方桌面助手\n• ChatGPT Desktop / Claude Desktop：适合日常问答、截图、文件、写作和资料整理。\n• 它们不是通用 API 客户端，不能直接把 DeepSeek / Kimi Key 填进去当模型。\n• 教程入口：/agent-install/chatgpt-desktop 和 /agent-install/claude-desktop。\n\n第六类：第三方多模型桌面客户端和本地模型应用\n• Cherry Studio / Chatbox / Msty / LobeHub：适合把 DeepSeek、Kimi、OpenAI Compatible 放到一个桌面界面里。\n• AnythingLLM：适合做个人或团队文档知识库。\n• LM Studio / Jan / GPT4All / Ollama：适合在本机下载模型，并给其他工具提供本地模型服务。\n• 教程入口：/agent-install/cherry-studio、/agent-install/msty、/agent-install/jan、/agent-install/lm-studio、/agent-install/ollama。\n\n小白安装顺序：先选一类 → 下载产品方官方或可信项目安装包 → 打开后先发一句中文测试 → 再配置模型/API → 最后做安装验证任务。",
      tools:["codex","openclaw","cherry-studio","chatbox","lm-studio","gemini-cli","qwen-code"],tips:"记住一句话：产品方官方 Agent 桌面端用官方账号，第三方 Agent 桌面端连 Agent 本体，IDE 插件装进编辑器，多模型客户端才填 DeepSeek/Kimi/OpenAI Compatible，本地模型应用负责把模型跑在自己电脑上。"},
