...
const xiaobaiDoubaoTtsConnection: AgentApiConnection = {
  name: "豆包 TTS 语音合成",
  badge: "自然语音输出",
  description: "让小白说话像在带你操作，而不是念稿。优先选自然对话音色，听起来顺了再继续做任务。新版控制台优先使用 API Key。",
  fields: [
...
export const agentInstallGuides: AgentInstallGuide[] = [
  {
    ...
    bestFor: [
      ...
      "想体验“我说话，小白听懂，然后告诉我下一步”的人。",
    ],
    requirements: [
      "Windows 10/11 电脑。",
      "小白AI网站会员账号。",
      "一套模型 API；要语音交互，再准备 ASR 语音识别和 TTS 语音合成。",
    ],
    installSteps: [
    ...
    firstPrompts: [
      ...
      "小白，用像朋友提醒我的语气说：现在还差哪一步才能开始语音操作电脑？",
      "小白，把我现在能用的模型、语音和桌面能力列成一张简单清单。",
      "小白，先做一个安全测试：只检查电脑状态，不要修改任何文件。",
    ],
    commonIssues: [
      ...
      { title: "不知道 API 去哪里申请", solution: "看本页“先选模型 API 或本地模型”区域。每一项都写了平台、入口、业务模块和要复制到小白哪里的字段。" },
      { title: "声音听起来不自然", solution: "先换 TTS 音色。选自然对话/亲和助手音色，不选新闻播报、客服机械音或情绪过重的音色；测试句要短，重点听停顿和尾音。" },
    ...
    postInstallSetup: [
      {
        title: "1. 先完成会员登录",
        why: "先把账号打通，后面聊天、语音、任务和桌面能力才不会散在不同地方。",
        steps: [
          "用小白AI网站会员账号登录桌面端。",
          "登录成功后再配置模型、语音和 TTS。",
...
codex`,
  ...
  },
  {
    ...
    minutes: "10-20 分钟",
    difficulty: "适合国内新手",
    tagline: "打开小白AI，复制命令，先让 OpenClaw 回第一句话，再把第一个任务交给它。",
    summary: "OpenClaw 安装页按“复制命令 -> 安装 -> 初始化 -> 中文测试”排好。你先跑通第一句话，确认模型能回答，再把项目或长期任务交给它。Windows 用户优先走 WSL2，成功率更高。",
