...
function platformSearchUrl(platform, title = '') {
  ...
  switch (platform) {
    ...
    case 'wechat':
      return `https://weixin.sogou.com/weixin?type=2&query=${q}`;
    case 'weibo':
...
function hotspotUrl(item = {}, platform = '') {
  const direct = String(item.url || '').trim();
  if (/^https?:\/\//i.test(direct)) return direct;
  return platformSearchUrl(item.platform || platform, item.text || item.title || '');
}

function hotspotKind(item = {}, platform = '') {
  const kind = String(item.kind || item.type || item.media_type || item.mediaType || '').toLowerCase();
  const url = hotspotUrl(item, platform);
  if (kind.includes('video') || /\.(mp4|webm|m3u8)(\?|#|$)/i.test(url)) return 'video';
  if (/youtube\.com|youtu\.be|bilibili\.com\/video|player\.bilibili\.com/i.test(url)) return 'video';
  return 'article';
}

function ensureHotspotReader() {
  let reader = document.getElementById('hs-reader');
  if (reader) return reader;
  reader = document.createElement('div');
  reader.id = 'hs-reader';
  reader.className = 'hs-reader';
  reader.innerHTML = `
    <div class="hs-reader-head">
      <div class="hs-reader-title" id="hs-reader-title">热点内容</div>
      <div class="hs-reader-actions">
        <button class="hs-reader-open" id="hs-reader-open" type="button">外部打开</button>
        <button class="hs-reader-close" id="hs-reader-close" type="button" title="关闭">×</button>
      </div>
    </div>
    <webview class="hs-reader-frame hs-reader-webview" id="hs-reader-webview" title="热点内置浏览器" partition="persist:xiaobai-hotspot"></webview>
    <iframe class="hs-reader-frame hs-reader-iframe" id="hs-reader-frame" title="热点内置浏览器兜底" sandbox="allow-scripts allow-same-origin allow-forms allow-popups" hidden></iframe>
    <div class="hs-reader-note" id="hs-reader-note">正在用小白内置浏览器打开。</div>
  `;
  document.body.appendChild(reader);
  reader.querySelector('#hs-reader-close')?.addEventListener('click', closeHotspotReader);
  reader.querySelector('#hs-reader-open')?.addEventListener('click', () => {
...
function closeHotspotReader() {
  const reader = document.getElementById('hs-reader');
  if (!reader) return;
  reader.classList.remove('visible');
  const webview = reader.querySelector('#hs-reader-webview');
  if (webview) webview.setAttribute('src', 'about:blank');
  const frame = reader.querySelector('#hs-reader-frame');
  if (frame) frame.src = 'about:blank';
  delete reader.dataset.url;
}

function blockAppProtocolNavigation(event) {
  const url = event?.url || event?.newURL || event?.targetUrl || '';
  if (url && !SAFE_WEB_URL_RE.test(url)) {
    event.preventDefault?.();
  }
}

function bindHotspotWebview(webview) {
  if (!webview || webview.dataset.boundHotspotWebview) return;
  webview.dataset.boundHotspotWebview = '1';
  webview.addEventListener('will-navigate', blockAppProtocolNavigation);
  webview.addEventListener('will-frame-navigate', blockAppProtocolNavigation);
  webview.addEventListener('will-redirect', blockAppProtocolNavigation);
  webview.addEventListener('new-window', blockAppProtocolNavigation);
  webview.addEventListener('did-fail-load', (event) => {
    const url = event?.validatedURL || event?.url || '';
    if (url && !SAFE_WEB_URL_RE.test(url)) event.preventDefault?.();
  });
}

function openHotspotReader({ url, title }) {
  if (!url) return;
  if (!SAFE_WEB_URL_RE.test(url)) return;
  const reader = ensureHotspotReader();
  reader.dataset.url = url;
  const titleEl = reader.querySelector('#hs-reader-title');
  const webview = reader.querySelector('#hs-reader-webview');
  const frame = reader.querySelector('#hs-reader-frame');
  const note = reader.querySelector('#hs-reader-note');
  if (titleEl) titleEl.textContent = title || '热点内容';
  if (note) note.textContent = '正在用小白内置浏览器打开。平台需要登录时，可以点“外部打开”。';
  if (webview?.tagName?.toLowerCase() === 'webview') {
    bindHotspotWebview(webview);
...
function openHotspotItem(item = {}, platform = '') {
  const url = hotspotUrl(item, platform);
  if (!url) return;
  const title = item.text || item.title || '热点内容';
  if (hotspotKind(item, platform) === 'video') {
    closeHotspotReader();
    window.bailongmaMedia?.showVideo?.({ url, title, autoplay: true });
    return;
  }
  openHotspotReader({ url, title });
}

function renderList(platform, listId, items, style = 'heat') {
  const ul = $(listId);
  if (!ul) return;
  if (!items.length) {
  ...
  ul.innerHTML = items.map((item) => {
    const { rank, text, heat, trend, isNew } = item;
    const openUrl = hotspotUrl(item, platform);
    const kind = hotspotKind(item, platform);
    ...
    return `<li class="hs-item hs-item-clickable">
      <button class="hs-item-link" type="button" data-platform="${escapeHtml(platform)}" data-url="${escapeHtml(openUrl)}" data-kind="${escapeHtml(kind)}" title="打开：${escapeHtml(text)}">
      <span class="hs-rank ${rankCls}">${escapeHtml(rank)}</span>
      <span class="hs-item-text">${escapeHtml(text)}${newBadge}</span>
      ${heatLabel}
      <span class="hs-trend ${trendCls}">${escapeHtml(trendIcon)}</span>
      </button>
    </li>`;
  }).join('');
}
...
function renderAllLists() {
  for (const [platform, config] of Object.entries(PLATFORM_CONFIG)) {
    renderList(platform, config.listId, hotspotLists[platform] || [], config.style);
  }
}
...
function normalizeHotspotItem(item, idx) {
  ...
  return {
    ...
    text,
    heat: item?.heat || item?.tag || '',
    trend: item?.trend || 'same',
    isNew: !!item?.isNew,
    url: item?.url || item?.share_url || item?.link || item?.jump_url || '',
    kind: item?.kind || item?.type || item?.media_type || item?.mediaType || '',
    source: item?.source || '',
  };
}
...
export async function initHotspot() {
  ...
  const panel = $('hotspot-panel');
  if (panel && !panel.dataset.hotspotClickBound) {
    panel.dataset.hotspotClickBound = '1';
    panel.addEventListener('click', (event) => {
      const button = event.target?.closest?.('.hs-item-link');
      if (!button) return;
      const platform = button.dataset.platform || '';
      const url = button.dataset.url || '';
      const item = (hotspotLists[platform] || []).find(candidate => hotspotUrl(candidate, platform) === url) || {
        platform,
        url,
        text: button.querySelector('.hs-item-text')?.textContent || '',
        kind: button.dataset.kind || '',
      };
      openHotspotItem(item, platform);
    });
  }
