...
export function extractPersonCardQuery(text = '') {
  ...
  for (const pattern of patterns) {
    ...
    if (match?.[1]) return match[1].replace(/^一下/, '').trim();
  }
  return '';
}
...
export function updatePersonCardFromAssistantText(text = '') {
  ...
  const knownFor = uniqueList([
    ...
    ...normalizeList(content.match(/淘宝|支付宝|阿里巴巴|天猫|菜鸟|达摩院|湖畔大学/g) || []),
  ]);
  const summary = content.length > 260 ? `${content.slice(0, 260)}...` : content;
  renderPersonCard({
    ...currentCard,
    summary: summary || currentCard.summary,
    knownFor,
    source: 'assistant',
    updatedAt: new Date().toISOString(),
  });
  reportPersonCardState(true, 'assistant_summary', currentCard);
  return true;
}
  ...
  try {
    ...
    setPersonCardMode(true, {
      ...
      card: {
        name: query,
        title: '人物卡片',
        summary: '暂时没有资料。可以让小白补充这个人的身份和代表作品。',
        knownFor: [],
        tags: ['待补充'],
        source: 'fallback',
        updatedAt: new Date().toISOString(),
...
export function initPersonCard() {
  renderPersonCard(currentCard || {
    name: '人物卡片',
    title: '待命',
    summary: '当你不认识某位公众人物时，小白会在这里弹出一张简短人物卡片。',
    knownFor: [],
    tags: ['standby'],
    source: 'standby',
  });
