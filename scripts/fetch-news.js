...
async function main() {
  ...
  const newItems = allNews.map(n=>
    `  {id:"${n.id}",title:${JSON.stringify(n.title)},summary:${JSON.stringify(n.summary.slice(0,200))},content:${JSON.stringify(n.content.slice(0,1000))},url:"${n.url}",source:"${n.source}",category:"${n.cat}",publishedAt:"${n.publishedAt}",importance:${n.imp},isAutoGenerated:true},\n`
  ).join("")
